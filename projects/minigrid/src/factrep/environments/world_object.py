...
class Goal(WorldObj):
    def __init__(self, kind: int, seen: bool = True):
        self.kind = kind
        self.seen = seen
        self.color = IDX_TO_COLOR[self.kind]
        assert self.color != "gray", "Goal color cannot be gray"
        super().__init__("goal", self.color)

    def can_overlap(self):
        return True

    def render(self, img):
        fill_coords(img, point_in_rect(0.18, 0.86, 0.18, 0.86), COLORS[self.color])
...
class Floor(WorldObj):
    ...
    def render(self, img):
        # left
        fill_coords(img, point_in_line(0.0, 0.0, 0.0, 1.0, r=0.18), COLORS[self.color])
        # bottom
        fill_coords(img, point_in_line(1.0, 1.0, 0.0, 1.0, r=0.14), COLORS[self.color])
        # top
        fill_coords(img, point_in_line(0.0, 0.0, 1.0, 0.0, r=0.18), COLORS[self.color])
        # right
        fill_coords(img, point_in_line(1.0, 0.0, 1.0, 1.0, r=0.14), COLORS[self.color])
...
class Box(WorldObj):
    ...
    def toggle(self, env, pos):
        ...
        env.grid.set(pos[0], pos[1], self.contains)  # type: ignore
        return True
